window.App = {
	...
	init : function() {
		...
		setOnlineStatus = function() {
			...
			if (window.navigator.onLine) {
				status = "<span style='color:green;'>" + window.Labels.onlineLabel + "</span>";
			} else {
				status = "<span style='color:red;'>" + window.Labels.offlineLabel + "</span>";
			}
	...
	loginBehavior : function(data) {
		...
		$('#login_button').click(function() {
			...
			$.ajaxOAUTH2Proxy({
				url : "devices",
				callback : function(d, status) {
					window.App.isLoggedIn = status == "success";
					if (window.App.isLoggedIn) {
						window.App.onLogin();
					}
					window.App.changeView(window.App.isLoggedIn ? "splash" : "login", window.App.isLoggedIn ? {} : {
	...
	processMarker : function(itemData, key, callback) {
		var submitData = {
		...
		$.ajaxOAUTH2Proxy({
			url : "http://api.rfinspired.com/update?key=" + key,
			data : submitData,
			type : "POST",
			callback : function(d) {
				//console.log(d);
		...
		setInterval(function() {
			if (window.navigator.onLine && window.App.isLoggedIn) {
				...
				$.each(que, function(i, itemData) {
					window.App.processMarker(itemData, key, window.App.emptyFun);
				});
	...
	addToMarkerSubmitQue : function(data, callback) {
		if (window.navigator.onLine) {
			var settings = window.App.getAppSettings();
			var key = settings["unique-key"];
			window.App.processMarker(data, key, callback);
			callback();
		} else {
			var que = window.App.getMarkerSubmitQue();
			que.push(data);
	...
	markerBehavior : function() {
		...
		$(".data-marker-name input").attr("placeholder", window.Labels.dataMarkerTextPalceholder).keyup(forceUpperCase);
		$(".data-marker-location input").attr("placeholder", window.Labels.dataMarkerLocationTextPalceholder).keyup(forceUpperCase);
		...
		function submit() {
			var settings = window.App.getAppSettings();
			if ($(".image_preview").val() != "" || $(".data-marker-name input").val() != "") {
				if (settings["unique-key"] && settings["unique-key"] != "") {
					navigator.geolocation.getCurrentPosition(function(position) {
						...
						window.App.addToMarkerSubmitQue({
							...
							values : postValues,
							alt : alt,
							marker : returnNullString($(".data-marker-name input").val()).toUpperCase(),
							lat : lat,
							lng : lng,
							scan : returnNullString($(".image_preview").val()).toUpperCase(),
							date : new Date()
						}, function() {
